...
USBPort *USBPort::Singleton = NULL;
...
///\return USB_SUCCESS or USB_FAILURE
/// // Param is num_trials try this number of times to open the device
int USBPort::Open(unsigned int Param)
{
    unsigned int i;
                            ...
                            bool checkFirstByte ) /// if true, sendData[0] is compared to rcvData[0] to see if they match
{
    ...
    {
        Open(0); // try to open the USB endpoint
        ...
        if ( ( sendData != NULL ) && ( sendLength > 0 ) )
            if ( !MPUSBWrite( epOut, sendData, sendLength, &sentDataLength, USB_TIMEOUT ) )
            {
                ...
                TCHAR errMsg[256];
                sprintf( errMsg, "MPUSBWrite() error %ld: failed: %s", dw, lpMsgBuf );
                string msg( errMsg );
...
//							Close();	// always close the USB endpoint after using it to allow other apps to open it
                ...
                else
                {
                    ...
                    TCHAR errMsg[256];
                    sprintf( errMsg, "MPUSBRead() error %ld: failed: %s", dw, lpMsgBuf );
                    string msg( errMsg );
                    GetErr().SimpleMsg( XSErrorMinor, msg );
                    // Close();    // always close the USB endpoint after using it to allow other apps to open it
                    return USB_FAILURE;
...
///< pointer to array of port numbers (not used)
/// return the number of XSUSB devices found
list<string> USBPort::ScanHardware(XSError *err)
{
    int numPorts = MPUSBGetDeviceCount( mpusbVidPid );
    list<string> Names;

    if (Singleton != NULL)
    {
        // Scan through all the USB ports.
...
/// /// that communicates with XSUSB
///\return a byte value from the EEPROM
unsigned int USBPort::ReadEeprom(unsigned int address) ///< EEPROM address to be written
{
    ...
    txLen = 5;
    rxLen = 1+5;
    if(SendRcvPacket(txBuf,txLen,rxBuf,&rxLen,true) != USB_SUCCESS)
    {
        return 0;
    }
    return rxBuf[5];
...
bool USBPort::WriteEeprom(unsigned int address, ///< EEPROM address to be written
