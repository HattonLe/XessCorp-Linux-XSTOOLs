...
bool PBusPort::Download(const char *hexfileName,	///< file containing data for the device on the peripheral bus
				...
				bool bigEndianBits)		///< if true, data is stored in Flash with most-significant bit in position 0
{
    ...
    string desc("RAM Download"), subdesc("Downloading ");
    progressGauge = new Progress(NULL, &err);
    if (NULL != progressGauge)
    {
        progressGauge->Setup(desc,subdesc.append(StripPrefix(hexfileName)),0,streamEndPos);

        status = Download(is,bigEndianBytes,bigEndianBits);
...
///\return true if operation was successful, false if not
bool PBusPort::Upload(const char *hexfileName,	///< dump uploaded data to this file
				...
				bool bigEndianBits)		///< if true, data is stored in Flash with most-significant bit in position 0
{
    ...
    ofstream os(hexfileName);  // otherwise open hex file
	if(os.fail() || os.eof()!=0)
	{ // error - couldn't open hex file
		err.SetSeverity(XSErrorMajor);
        err << "could not open " << hexfileName << "\n";
    ...
    string desc("RAM Upload"), subdesc("Uploading ");
    progressGauge = new Progress(NULL, &err);
    if (NULL != progressGauge)
    {
        progressGauge->Setup(desc,subdesc.append(StripPrefix(hexfileName)),loAddr,hiAddr);

        status = Upload(os,format,loAddr,hiAddr,bigEndianBytes,bigEndianBits);
