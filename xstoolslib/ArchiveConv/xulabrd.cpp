...
static precision_timer prec_timer_xula = 0;
#define TIME_IT(stmnt)\
{\
	prec_timer_xula.start();\
	stmnt;\
	prec_timer_xula.stop();\
}
#else
...
XuLABoard::XuLABoard(void)
{
	XSError* err = new XSError(cerr);		// create error-reporting channel
	brdModel = NULL;
	Setup(err,"XULA-200",0);				// use a default board model and USB port at this point
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
void XuLABoard::SetFlags(unsigned long f)
{
	flags = f;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned long XuLABoard::GetFlags(void)
{
	return flags;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::LoadAndTestJtagPortPtrs(void)
{
	...
	return (jtagportFPGA != NULL); // return true if port pointer is non-NULL
}


//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::Setup(XSError* err, const char* model, unsigned int prtNum)
{
	...
	{
		if(_stricmp(brdModel,brdInfo[brdIndex].brdModel) == 0)
			break;
	}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::Configure(string& fileName)
{
	...
	if(suffix=="BIT")
	{
		...
		DEBUG_STMT("Time to download bitstream" << prec_timer_xula << " seconds")
		return status;
	}
	
	// not a .BIT, so report the error
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::ConfigureInterface(string& fileName)
{
	if(fileName.length()==0)
		return true; // nothing to do, so it must be OK
	...
	// No CPLD on the XuLA board, so report the error
	string msg = (string)"There is no CPLD on the " + brdModel + (string)"!!\n";
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::DownloadRAM(string& fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	while(is.eof()==0)
	{
		...
		if((loAddr != nextAddr && byteIndex > 0) || byteIndex+numBytes > DWNLD_BUFFER_SIZE)
		{
			...
			DEBUG_STMT("Time to download " << dec << 8*byteIndex << " bits = " << prec_timer_xula << " seconds")
			DEBUG_STMT("Transfer rate = " << dec << 8*byteIndex / prec_timer_xula << " bps")
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::UploadRAM(string& fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,
			bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)			
{
	...
	DEBUG_STMT("Time to upload " << dec << 8*numBytes << " bits = " << prec_timer_xula << " seconds")
	DEBUG_STMT("Transfer rate = " << dec << 8*numBytes / prec_timer_xula << " bps")
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::ReadRAM(unsigned int addr, unsigned int* data, bool bigEndianBytes, bool bigEndianBits)	
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::WriteRAM(unsigned int addr, unsigned int data, bool bigEndianBytes, bool bigEndianBits)	
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::DownloadFlash(string& fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	string suffix = GetSuffix(fileName);
	if(suffix!="HEX" && suffix!="EXO" && suffix!="XES" && suffix!="MCS" && suffix!="BIT")
	{
		brdErr->SimpleMsg(XSErrorMajor,(string)"Only .HEX, .MCS, .EXO, .XES or.BIT files can be downloaded into the Flash on the " + brdModel + (string)"!!\n");
		return false;
	}
	...
	// Enable programming of the Flash.
	jtagportFPGA->SetFlashEnable(1);
...
//	}
	...
	while(is.eof()==0)
	{
		...
		if(numBytes % stride)
		{
			...
			jtagportFPGA->SetFlashEnable(0); // Disable programming of the Flash.
			return false;
		}
		loAddr = hx.GetAddress();
		if(loAddr & ~addrMask)
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::UploadFlash(string& fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,	
			bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)				
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	// Enable programming of the Flash.
	jtagportFPGA->SetFlashEnable(1); // Enable reading of the Flash.
	...
	if((hiAddr-loAddr+1) % stride)
	{
		...
		jtagportFPGA->SetFlashEnable(0); // Disable reading of the Flash.
		return false;
	}
	if(loAddr & ~addrMask)
	{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::SetFreq(int div, bool extOscPresent)
{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::SetupAudio(int *reg)
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::SetupVideoIn(string& fileName)
{
	return false;
}
...
try_test_loop:
	...
	else if(data == opFailed)
	{
		...
		msg += "2) Is your board sitting on a non-conductive surface?\n";
		brdErr->SimpleMsg(XSErrorMajor,msg);
		return false;
	}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned char XuLABoard::ApplyTestVectors(unsigned char singleVector, unsigned char mask, unsigned char *vector,
											unsigned char *response, unsigned int numVectors)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD

	string msg = (string)"\nYour " + brdModel + (string)" does not support test vectors!\n";
	brdErr->SimpleMsg(XSErrorMajor,msg);
	return false;
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned char XuLABoard::GetTestVector(void)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::DownloadRAMFromIntArray(unsigned *intArray, unsigned address, unsigned numInts)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	jtagportFPGA->GoThruTAPSequence(PauseDR,Exit2DR,ShiftDR,-1);
	TIME_IT(jtagportFPGA->SendRcvBitstream(data,null))
	DEBUG_STMT("Time to download " << dec << 16*numInts << " bits = " << prec_timer_xula << " seconds")
	DEBUG_STMT("\tTransfer rate = " << dec << 16*numInts / prec_timer_xula << " bps")
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::UploadRAMToIntArray(unsigned *intArray, unsigned address, unsigned numInts)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	jtagportFPGA->GoThruTAPSequence(PauseDR,Exit2DR,ShiftDR,-1);
	TIME_IT(jtagportFPGA->SendRcvBitstream(null,data))
	DEBUG_STMT("Time to upload " << dec << 16*numInts << " bits = " << prec_timer_xula << " seconds")
	DEBUG_STMT("\tTransfer rate = " << dec << 16*numInts / prec_timer_xula << " bps")
