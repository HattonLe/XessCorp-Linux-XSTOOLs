...
/// Instantiate an XCVPort object on a given parallel port.
XCVPort::XCVPort(void)
{
	progressGauge = NULL;
}
...
/// Instantiate an XCVPort object on a given parallel port.
XCVPort::XCVPort(XSError* e, ///< pointer to error reporting object
				   ...
				   unsigned int posDin,	///< serial config. data input
...
{
	progressGauge = NULL;
	Setup(e,portNum,invMask,posCCLK,posPROG,posDin,posDlo,posDhi,posDONE);
}


/// Setup an XCVPort object on a given parallel port.
///\return true if the operation was a success, false otherwise
bool XCVPort::Setup(XSError* e, ///< pointer to error reporting object
				   unsigned int portNum, ///< parallel port number
				   unsigned int invMask, ///< inversion mask for the parallel port
				   unsigned int posCCLK, ///< bit position in parallel port of CCLK pin
				   unsigned int posPROG, ///< bit position in parallel port of PROGRAM pin
...
string XCVPort::GetChipType(istream& is) ///< stream that delivers the bitstream
{
	XSError& err = GetErr();
...
/// Get the chip identifier from the FPGA bitstream.
///\return the chip identifier
string XCVPort::GetChipType(string& bitfileName)	///< file containing the FPGA bitstream
{
	XSError& err = GetErr();
	
	if(bitfileName.compare("")==0)
		return false; // exit if no BIT file was given
...
void XCVPort::InitConfigureFPGA(void)
{
	SetPROG(1);
	SetCCLK(0);
}
...
void XCVPort::EnableFastDownload(bool t)	///< if true, enable fast parallel download, otherwise do serial download
{
	fastDownload = t;
}
...
void XCVPort::ConfigureFPGA(unsigned char b)	///< configuration byte from FPGA bitstream
{
	if(fastDownload)
...
///\return true if the operation was a success, false otherwise
bool XCVPort::ConfigureFPGA(istream& is)	///< stream that delivers the FPGA bitstream
{
	XSError& err = GetErr();
...
///\return true if the operation was a success, false otherwise
bool XCVPort::ConfigureFPGA(string& fileName)	///< file with FPGA bitstream
{
	XSError& err = GetErr();
