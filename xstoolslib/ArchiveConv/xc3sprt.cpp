...
/// Get the chip identifier from the FPGA bitstream.
///\return the chip identifier
string XC3SPort::GetChipType(string& bitfileName)	///< file containing the FPGA bitstream
{
	XSError& err = GetErr();
...
///\return true if the operation was a success, false otherwise
bool XC3SPort::ConfigureFPGA(istream& is)	///< stream that delivers the FPGA bitstream
{
	XSError& err = GetErr();
...
///\return true if the operation was a success, false otherwise
bool XC3SPort::ConfigureFPGA(string& fileName)	///< file with FPGA bitstream
{
	XSError& err = GetErr();
