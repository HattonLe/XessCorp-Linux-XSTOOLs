...
// USERCODE strings for various circuits programmed into the CPLD
...
static const string xessjtagCode            = "<5>!";
...
XSJTAGBoard::XSJTAGBoard(void)
{
	XSError* err = new XSError(cerr);		// create error-reporting channel
	brdModel = NULL;
	Setup(err,"XSA-3S1000",0);				// use a default board model and USB port at this point
}
...
XSJTAGBoard::~XSJTAGBoard(void)
{
	// close the ports but don't delete them.  we just keep pointers to them; we don't own them.
	if(LoadAndTestJtagPortPtrs())
	{
		jtagportFPGA->Close();
		jtagportCPLD->Close();
	}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
void XSJTAGBoard::SetFlags(unsigned long f)
{
	flags = f;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned long XSJTAGBoard::GetFlags(void)
{
	return flags;
}
...
//##########################################################################################################
// 
string XSJTAGBoard::GetUSERCODE(void)
{
	if(!LoadAndTestJtagPortPtrs())
		return string("");
	...
	jtagportCPLD->InitTAP();						// initialize the JTAG TAP controller
	jtagportCPLD->LoadBSIRthenBSDR(bsir,null,bsdr);	// get the ID code from the chip
	char *code = (char*)bsdr.ToCharString();		// change the ID code into a character string
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::LoadAndTestJtagPortPtrs(void)
{
	// get pointers to the port for this board
	jtagportFPGA = GetPort(portType,portNum,1);
	if(jtagportFPGA == NULL)
		brdErr->SimpleMsg(XSErrorMajor,"unable to open port to FPGA!\n");
	jtagportCPLD = GetPort(portType,portNum,2);
	if(jtagportCPLD == NULL)
		brdErr->SimpleMsg(XSErrorMajor,"unable to open port to CPLD!\n");
	return (jtagportCPLD != NULL) && (jtagportFPGA != NULL); // return true if both port pointers are non-NULL
}


//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::Setup(XSError* err, const char* model, unsigned int prtNum)
{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::Configure(string& fileName)
{
	if(fileName.length()==0)
		return true; // nothing to do, so it must be OK

	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD

	// Check the suffix of the bitstream file.
	string suffix = GetSuffix(fileName);
	if(suffix=="BIT")
	{
		// Check the CPLD USERCODE to see if the CPLD will allow downloading of FPGA bitstreams.
		if(GetUSERCODE() != xessjtagCode)
		{
			// The CPLD doesn't allow FPGA bitstream downloads, so reprogram it so it does. 
			if(ConfigureInterface(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].testIntfcBitstreamFile)) == false)
			{
				brdErr->SimpleMsg(XSErrorMajor,"Unable to configure CPLD interface!\n");
				return false;
			}
			// Check the CPLD USERCODE to see if the CPLD was actually reprogrammed.
			if(GetUSERCODE() != xessjtagCode)
			{
				brdErr->SimpleMsg(XSErrorMajor,"Unable to configure CPLD interface to allow downloading the FPGA configuration bitstream!\n");
				return false;
			}
		}
		// Now download the configuration bitstream to the FPGA.
	...
	*brdErr << "Only .BIT or .SVF files can be downloaded into the FPGA on the " << brdModel << " Board\n";
	brdErr->EndMsg();
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::ConfigureInterface(string& fileName)
{
	if(fileName.length()==0)
		return true; // nothing to do, so it must be OK
	...
	// Check the suffix of the bitstream file. .SVF files go to the interface CPLD.
	string suffix = GetSuffix(fileName);
	if(suffix=="SVF")
	{
		jtagportCPLD->SenseInverters();	// Sense any inverters on the interface CPLD JTAG pins.
		jtagportCPLD->StartBuffer();	// Buffer JTAG commands for faster download thru USB
		// Create and display progress gauge.
		string desc("Download SVF"), subdesc("Downloading "+StripPrefix(string(fileName)));
		Progress *progressGauge = new Progress(brdErr,desc,subdesc,0,100,true);
		progressGauge->Report(0);
		// Download the SVF configuration file into the CPLD.
		bool status = jtagportCPLD->DownloadConfigFile(fileName);
		jtagportCPLD->FlushBuffer();	// Flush buffered commands.
		// Get rid of the progress gauge.
		if(progressGauge != NULL)
			delete progressGauge;
		return status;
	}
	
	// not a .SVF file, so report the error
	brdErr->SetSeverity(XSErrorMajor);
	*brdErr << "Only SVF files can be downloaded into the CPLD on the " << brdModel << " Board\n";
	brdErr->EndMsg();
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::DownloadRAM(string& fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	{
		brdErr->SimpleMsg(XSErrorMajor,(string)"Only .HEX, .MCS, .EXO or .XES files can be downloaded into the RAM on the " + brdModel + (string)"!!\n");
		return false;
	}
	...
	{ // error - couldn't open hex file
		brdErr->SetSeverity(XSErrorMajor);
		*brdErr << "Could not open " << fileName.c_str() << "\n";
		brdErr->EndMsg();
		return false;
	}
	...
	if(doStart && !ramIntfcAlreadyLoaded)
	{
		...
		if(Configure(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].ramBitstreamFile)) == false)
		{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::UploadRAM(string& fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::ReadRAM(unsigned int addr, unsigned int* data, bool bigEndianBytes, bool bigEndianBits)	
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::WriteRAM(unsigned int addr, unsigned int data, bool bigEndianBytes, bool bigEndianBits)	
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::DownloadFlash(string& fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	if(suffix!="HEX" && suffix!="EXO" && suffix!="XES" && suffix!="MCS")
	{
		brdErr->SimpleMsg(XSErrorMajor,(string)"Only .HEX, .MCS, .EXO or .XES files can be downloaded into the Flash on the " + brdModel + (string)"!!\n");
		return false;
	}
	...
	if(doStart && !flashIntfcAlreadyLoaded)
	{
		...
		if(Configure(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].flashBitstreamFile)) == false)
		{
...
//	delete [] bytes;
	...
	if(doEnd)
	{
		if(Configure(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].flashConfigBitstreamFile)) == false)
		{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::UploadFlash(string& fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,	
			bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)				
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::SetFreq(int div, bool extOscPresent)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	string instructions;
	instructions = "Before setting the XSA-50 or XSA-100 Board frequency you must:\n\n";
	...
	int status = jtagportFPGA->SetOscFrequency(div,extOscPresent);

	if(!status)
	{
		string msg("Error while programming the oscillator frequency!\n");
		brdErr->SimpleMsg(XSErrorMajor,msg);
		return false;
	}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::SetupAudio(int *reg)
{
	return false;
}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::SetupVideoIn(string& fileName)
{
	return false;
}
...
try_test_loop:
	...
	if(!testIntfcAlreadyLoaded)
	{
		...
		if(ConfigureInterface(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].testIntfcBitstreamFile)) == false)
		{
			brdErr->SimpleMsg(XSErrorMajor,"Error configuring CPLD with self-test interface circuit!\n");
			return false;
		}
		// download the diagnostic circuit to the board
		if(Configure(string(brdInfo[brdIndex].port[PORTTYPE_USBJTAG].testBitstreamFile)) == false)
		{
			brdErr->SetSeverity(XSErrorMajor);
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned char XSJTAGBoard::ApplyTestVectors(unsigned char singleVector, unsigned char mask, unsigned char *vector,
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
unsigned char XSJTAGBoard::GetTestVector(void)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::DownloadRAMFromIntArray(unsigned *intArray, unsigned address, unsigned numInts)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XSJTAGBoard::UploadRAMToIntArray(unsigned *intArray, unsigned address, unsigned numInts)			
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
