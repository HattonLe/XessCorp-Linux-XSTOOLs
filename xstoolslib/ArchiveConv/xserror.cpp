...
#ifdef _WINDOWS
		if(!batch) AfxMessageBox(storedMsg.c_str(),MB_ICONSTOP);
#endif
		exit(1);
		break;
	case XSErrorMajor:
...
#ifdef _WINDOWS
		if(!batch) AfxMessageBox(storedMsg.c_str(),MB_ICONEXCLAMATION);
#endif
		storedMsg = "";
...
#ifdef _WINDOWS
		if(!batch) AfxMessageBox(storedMsg.c_str(),MB_ICONINFORMATION);
#endif
		storedMsg = "";
		SetState(XSErrorInitial);
		break;
