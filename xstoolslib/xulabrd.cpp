...
bool XuLABoard::Setup(XSError* err, const char* model, unsigned int prtNum)
{
	...
	{
        if(ConvertToUpperCase(brdModel).compare(ConvertToUpperCase(brdInfo[brdIndex].brdModel)) == 0)
			break;
	}
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::Configure(const char *fileName)
{
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::ConfigureInterface(const char *fileName)
{
    if(strlen(fileName)==0)
		return true; // nothing to do, so it must be OK
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::DownloadRAM(const char *fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::UploadRAM(const char *fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::DownloadFlash(const char *fileName, bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
	...
	if(suffix!="HEX" && suffix!="EXO" && suffix!="XES" && suffix!="MCS" && suffix!="BIT")
	{
        ...
        Msg = "Only .HEX, .MCS, .EXO, .XES or.BIT files can be downloaded into the Flash on the ";
        Msg += brdModel;
        Msg += "!!\n";
        brdErr->SimpleMsg(XSErrorMajor,Msg);
        return false;
...
//##########################################################################################################
// Look at xsboard.h for a description of the interface.
bool XuLABoard::UploadFlash(const char *fileName, const char* format, unsigned int loAddr, unsigned int hiAddr,
			bool bigEndianBytes, bool bigEndianBits, bool doStart, bool doEnd)				
{
	if(!LoadAndTestJtagPortPtrs())
		return false; // some problem occurred with USB ports to FPGA or CPLD
