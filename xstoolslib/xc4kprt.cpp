...
/// Get the chip identifier from the FPGA bitstream.
///\return the chip identifier
string XC4KPort::GetChipType(const char *bitfileName)	///< file containing the FPGA bitstream
{
	XSError& err = GetErr();
...
///\return true if the operation was a success, false otherwise
bool XC4KPort::ConfigureFPGA(const char *fileName)	///< file with FPGA bitstream
{
    bool status;
