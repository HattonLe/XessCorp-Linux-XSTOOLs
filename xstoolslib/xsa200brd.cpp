...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::Configure(const char *fileName, bool *UserCancelled)
{
    bool status;
...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::ConfigureInterface(const char *fileName, bool *UserCancelled)
{
    bool status;
...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::DownloadRAM(const char *fileName, bool bigEndianBytes,
...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::UploadRAM(const char *fileName, const char* format,
    ...
    if (!ram.ReadRAM(addr,data,bigEndianBytes,bigEndianBits))
    {
        ...
        else
        {
            ...
            if (CheckChipID())
            {
                ...
                status = Configure(brdInfo[brdIndex].port[PORTTYPE_LPT].ramBitstreamFile, UserCancelled);
                if (!status)
                {
                    errMsg.SimpleMsg(XSErrorMajor,"Error downloading RAM interface circuit!!\n");
                }
                else
                {
                    if (!UserCancelled)
                    {
                        ...
                        status = ram.ReadRAM(addr, data, bigEndianBytes, bigEndianBits);	// now try reading the RAM again
                    }
                }
            }
        }
...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::DownloadFlash(const char *fileName, bool bigEndianBytes,
...
// Look at xsboard.h for a description of the interface.
bool XSA200Board::UploadFlash(const char *fileName, const char* format,
				unsigned int loAddr, unsigned int hiAddr,
				bool bigEndianBytes, bool bigEndianBits,	
				bool doStart, bool doEnd)
{
...
bool XSA200Board::Test(void)
{
	...
	if(loadTestIntfc)
	{
        ...
        if(ConfigureInterface(brdInfo[brdIndex].port[PORTTYPE_LPT].testIntfcBitstreamFile) == false)
		{
